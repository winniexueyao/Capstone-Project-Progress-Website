...
export default function ProgressPage() {
  ...
  const getProjectInfo = () => {
    ...
    const project = projects[0]; // 假设只有一个主项目
  ...
  const calculateProjectProgress = () => {
    ...
    const totalWeight = milestones.reduce((sum, milestone) => sum + getMilestoneWeight(milestone), 0);
    const weightedProgress = milestones.reduce((sum, milestone) => {
      return sum + (milestone.progress * getMilestoneWeight(milestone));
    }, 0);
    
    return Math.round(weightedProgress / totalWeight);
  };

  // 获取里程碑权重（可以根据需要调整权重计算方法）
  const getMilestoneWeight = (milestone) => {
  ...
  const calculateTimeProgress = () => {
    ...
    return Math.round((projectInfo.daysElapsed / projectInfo.daysTotal) * 100);
  };
  ...
  const calculateTimelinePosition = (startDate, endDate) => {
    const projectInfo = getProjectInfo();
    if (!projectInfo) return { left: '0%', width: '0%' };
    
    const projectStart = new Date(projectInfo.start_date);
    const projectEnd = new Date(projectInfo.end_date);
          ...
          <Card>
            <CardHeader>
              ...
              <p className="text-gray-500">
                项目周期: {formatDate(projectInfo.start_date)} - {formatDate(projectInfo.end_date)}
              </p>
            </CardHeader>
            <CardContent>
              ...
              <div className="relative">
                ...
                <div className="relative h-16">
                  {milestones.map((milestone) => {
                    const position = calculateTimelinePosition(milestone.start_date || projectInfo.start_date, milestone.due_date);
                    return (
                      <div 
                        key={milestone.id}
                ...
                <div className="flex justify-between text-sm text-gray-500 mt-2">
                  <span>{formatDate(projectInfo.start_date)}</span>
                  <span>{formatDate(projectInfo.end_date)}</span>
                </div>
              </div>
            </CardContent>
          </Card>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                      ...
                      <p className="font-medium">
                        {formatDate(milestone.start_date || projectInfo.start_date)} - {formatDate(milestone.due_date)}
                      </p>
                      <p className="text-sm text-gray-500 mt-1">
                        持续 {getDaysBetween(milestone.start_date || projectInfo.start_date, milestone.due_date)} 天
                      </p>
                    </div>
                    <div>
